...
const Navbar = () => {
  ...
  useEffect(() => {
    ...
    return (
        <nav id='navbar' className={`bg-transparent-500 dark:bg-transparent-900 fixed w-full z-20 top-0 start-0 dark:border-transparent-600 transition-all duration-300 ${show ? 'translate-y-0' : '-translate-y-full'}`}>
           <div className="max-w-screen-xl flex flex-wrap items-center justify-between mx-auto p-4">
            ...
            <div className="hidden md:flex container mx-auto justify-around">
        ...
        {isMobileMenuOpen && (
            <div
            className={`fixed top-0 left-0 rounded-r-lg bg-slate-900 text-white z-40 transition-all duration-300 ease-in-out ${
              isMobileMenuOpen ? 'top-[calc(4rem+2px)] h-auto opacity-100' : 'top-[-100%] h-0 opacity-0'} overflow-hidden`}
              style={{top: '4rem'}}
          >
